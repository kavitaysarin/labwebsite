...
- **Provider = Web3Forms** (chosen). Delivery inbox = **`ksarin@stanford.edu`** (chosen). The form is fully built (fields, validation, submit/success/error states, honeypots, card→form preselect, a11y) but runs in **safe draft mode** until a real end-to-end send is confirmed. **REMAINING LAUNCH STEP:** put the Web3Forms key (registered to ksarin@stanford.edu) in `NEXT_PUBLIC_WEB3FORMS_KEY` (`.env.local` / prod build secret), and confirm a **real test message is delivered end-to-end** — only then does the draft notice disappear and the page become final. Full guide: `CONTACT_FORM_MAINTENANCE.md`.
