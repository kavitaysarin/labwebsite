...
- **Digital Sensor image** (Research → Skin as a Digital Sensor) is now a **cNF-Vision software screenshot** (`digital-sensor.jpg`, supplied 2026-07-01) — a **patient skin photograph** with AI-detected/classified cutaneous neurofibromas and per-lesion stage counts. It carries a **"DermNet" watermark** on the underlying photo. **Confirm patient consent / de-identification AND reuse rights for the base image (DermNet licensing)** before launch. (Replaced the earlier 3×3 `cnfvision.gif` montage frame.)
- **3D Molecular Pathology** is now a **looping video** (lab-supplied `3Dpathology.mov`, transcoded 2026-07-01 to `molecular-3d.mp4`/`.webm` ≈0.4 MB + a poster frame). Muted autoplay loop; pauses on the poster for reduced-motion users. Confirm it is the lab's own footage and cleared for web use. (The previous static figure `molecular-3d.jpg` was removed.)
...
- **Optical Imaging** image (Technologies page) is a **product photo of the DAMAE Medical "deepLive" device** (`deeplive.webp`, 800×800 on white, shown `contain`; lab-supplied 2026-07-01). This is a **commercial product/marketing photo** — **confirm reuse rights** from DAMAE Medical. (The homepage Optical Imaging card still uses the previous `optical-imaging.jpg` OCT scan.)
- **AI & Computational Analysis** (Technologies page) is now the **animated cNF-Vision montage** (lab-supplied `cnfvision.gif` — 24 MB — transcoded 2026-07-01 to `cnfvision.mp4`/`.webm` ≈0.2 MB + poster; muted autoplay loop, reduced-motion falls back to the poster). It shows **patient skin photographs** with segmented cutaneous neurofibromas — **confirm patient consent / de-identification for public display** (same asset as the Research digital-sensor frame).
- **Optical Clearing** program (Technologies) — the term "KLEAR" was removed from the name, lead, and alt text per request (2026-07-01); the description was rewritten ("optical clearing technologies use refractive index manipulation…" — verb corrected from the supplied "uses" for plural agreement). The `klear.jpg` before/after panel image is unchanged.
...
- **Curated subset (by design):** the page is a highlight (~40 papers after the 2026-07-01 Skin Cancer and Genetics expansion), not the full archive; the complete list is the PubMed author link. The selection can be edited at any time via `PUBLICATIONS_MAINTENANCE.md`.
- **Skin Cancer and Genetics expansion (2026-07-01):** added **20 publications** from a supplied DOI list, each resolved to a PMID and verified via PubMed (title/journal/year/first author). **Two supplied DOIs were incomplete and could not be resolved — `10.1158/1078-0432` and `10.1038/jid.2015` (truncated, missing the article suffix); please resend the full DOIs.** Two supplied DOIs were already on the page (`10.1126/sciadv.adi5794` = Optical Imaging Winetraub; `10.1126/scitranslmed.ade1844` = NFX-179) and were not duplicated. Note the set includes cancer-genetics papers beyond skin (BAP1/BARD1 mesothelioma, PD-1 melanoma) per the supplied list — confirm the section grouping.
...
- **Roster changes (2026-07-01):** removed **Victoria Harbour** and **Sahar Caravan**; added **Angelina Chekmareva** (Research Administrator, Staff) and **Kat Israel** (Senior Associate Director, Major Gifts, Staff); **Sally Glaser** (now PhD; Volunteer, Scientific Editor) and **Margarita Golod** (now MBA; Volunteer, Outreach and Engagement) gained lab-supplied photos + bios; **Audris Chiang** (surname corrected from "Chang"; +MD). Confirm **Margarita's email** (`margarita.golod@gmail.com` — supplied as "maragarita…", likely typo) and the **Chiang** spelling.
- **Added 2026-07-01 (2 more):** **Chris Sadee** (AI Data Scientist, Staff; sadee@stanford.edu) and **Akash Dhawan** (M.D.–Ph.D. Student, Medical Engineering; Trainees; akash.s.dhawan@gmail.com) — both with lab-supplied photos. Akash's standalone **"PhD" credential was dropped** because it conflicts with "M.D.–Ph.D. Student" (a current student hasn't earned the PhD); if he already holds a PhD, restore the credential and change the role (e.g., "M.D. Student"). Team is now 18.
- **No headshot (initials fallback):** none — Angelina Chekmareva's photo was added 2026-07-01. Angelina, Haniya, and Jeffrey were re-cropped tighter (face centered) to match the others.
...
- **Helen Bronte-Stewart and Lorinda Chung REMOVED (2026-07-01, "may add back later").** Their entries were taken out of `collaborators.ts`; their photos remain in `public/images/collaborators/` for easy re-add. **Christina Curtis added** (Imaging/Engineering/Data Science; RZ Cao Professor of Medicine, Genetics, and Biomedical Data Science) — official Stanford CAP photo; confirm reuse permission.
