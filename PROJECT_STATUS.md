...
- Branch **`feature/remaining-pages-draft`** (NOT `main`, NOT merged, NOT deployed). DRAFT (not approved) versions built, one commit each:
  ...
  - **Contact** — refined 2026-06-30 (**still DRAFT — needs a real test send**): full contact form wired to **Web3Forms** (delivery → `ksarin@stanford.edu`), reading a public key from `NEXT_PUBLIC_WEB3FORMS_KEY` (kept out of git). Fields (name/email/affiliation/reason/message), inline accessible validation, honeypots + too-fast trap, Sending/Success/Error states (success only on provider confirmation — never faked), "How We Can Help" cards that preselect the reason + scroll to the form, and a Patient/Clinical **block-and-redirect** to Stanford Health Care. Runs in safe draft mode until the key + a confirmed end-to-end delivery land. Guide: `CONTACT_FORM_MAINTENANCE.md`.
